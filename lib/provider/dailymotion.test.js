...
test('Dailymotion: undefined', () => {
  ...
  expect(newParser().create({ videoInfo: { provider: 'dailymotion' } })).toBe(
    undefined,
  );
  expect(
    newParser().create({
      videoInfo: { provider: 'dailymotion', mediaType: 'video' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
...
test('Dailymotion: urls', () => {
  ...
  testUrls(newParser(), {
    ...
    urls: [
      'http://www.dailymotion.com/video/x1e2b95?start=10',
