...
test('TeacherTube: undefined', () => {
  ...
  expect(newParser().parse('https://teachertube.com/asd/123123')).toBe(
    undefined,
  );
  expect(newParser().parse('https://teachertube.com/video/-')).toBe(undefined);
  expect(newParser().create({ videoInfo: { provider: 'teachertube' } })).toBe(
    undefined,
  );
  expect(
    newParser().create({
      videoInfo: { provider: 'teachertube', mediaType: 'video' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
  ...
  expect(
    newParser().create({
      videoInfo: { provider: 'teachertube', mediaType: 'audio' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
  ...
  expect(
    newParser().create({
      videoInfo: { provider: 'teachertube', mediaType: 'channel' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
  ...
  expect(
    newParser().create({
      videoInfo: { provider: 'teachertube', mediaType: 'document' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
  ...
  expect(
    newParser().create({
      videoInfo: { provider: 'teachertube', mediaType: 'group' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
  ...
  expect(
    newParser().create({
      videoInfo: { provider: 'teachertube', mediaType: 'collection' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
