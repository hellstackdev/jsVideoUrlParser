...
test('Loom: undefined', () => {
  ...
  expect(newParser().create({ videoInfo: { provider: 'loom' } })).toBe(
    undefined,
  );
  expect(
    newParser().create({ videoInfo: { provider: 'loom', mediaType: 'video' } }),
  ).toBe(undefined);
  expect(
    newParser().create({
      videoInfo: { provider: 'loom', mediaType: 'video' },
      format: 'embed',
    }),
  ).toBe(undefined);
  expect(
...
test('Loom: urls', () => {
  testUrls(newParser(), {
    ...
    urls: [
      'https://loom.com/share/9206412c4aba41bcabd05b4d100911ba',
