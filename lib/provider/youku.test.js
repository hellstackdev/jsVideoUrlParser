...
test('Youku: undefined', () => {
  ...
  expect(newParser().create({ videoInfo: { provider: 'youku' } })).toBe(
    undefined,
  );
  expect(
    newParser().create({
      videoInfo: { provider: 'youku', mediaType: 'video' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
...
test('Youku: urls', () => {
  testUrls(newParser(), {
    ...
    formats: {
      ...
      static:
        'http://static.youku.com/v1.0.0638/v/swf/loader.swf?VideoIDS=XMTQ3OTM4MzMxMg',
    },
    urls: [
