...
test('SoundCloud: undefined', () => {
  ...
  expect(newParser().parse('https://soundcloud.com/julian-hangst-rfer/')).toBe(
    undefined,
  );
  expect(newParser().create({ videoInfo: { provider: 'soundcloud' } })).toBe(
    undefined,
  );
  expect(
    newParser().create({
      videoInfo: { provider: 'soundcloud', mediaType: 'track' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
      videoInfo: { provider: 'soundcloud', mediaType: 'playlist' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
      videoInfo: { provider: 'soundcloud', mediaType: 'apitrack' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
      videoInfo: { provider: 'soundcloud', mediaType: 'apiplaylist' },
    }),
  ).toBe(undefined);
  expect(
    newParser().create({
...
test('SoundCloud: track urls', () => {
  ...
  testUrls(newParser(), {
    ...
    formats: {
      long:
        'https://soundcloud.com/julian-hangst-rfer/odsf0dif92w3j_adfw-edf-1-asdf-1#t=30',
      embed: undefined,
    },
    urls: [
