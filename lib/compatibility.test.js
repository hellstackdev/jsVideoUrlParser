...
test('compat: parseProvider host matching behavior remains stable', () => {
  ...
  expect(localParser.parseProvider('https://www.youtube.com/watch?v=HRb7B9fPhfA')).toBe('youtube');
  expect(localParser.parseProvider('//m.vimeo.com/123456')).toBe('vimeo');
  expect(localParser.parseProvider('http://player.twitch.tv/?video=v123')).toBe('twitch');
});
